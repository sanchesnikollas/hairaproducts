...
def enrich(brand: str, limit: int, dry_run: bool):
    """Re-process catalog products to recover INCI and 'como usar' via LLM fallback.

    Targets catalog products (gold_status='catalog') missing INCI OR usage_instructions,
    re-fetches each product's own page, and fills both fields (deterministic first,
    LLM fallback). This is where Gold conversions come from — see src/enrichment/enricher.py.
    """
    ...
    with SASession(engine) as session:
        stats = enrich_brand(
            session, brand,
            llm=llm, browser=browser, limit=limit, dry_run=dry_run,
            log=lambda m: click.echo(f"    {m}"),
        )
    ...
    click.echo(f"{'='*60}")
    click.echo(f"Candidates (catalog, missing INCI/usage): {stats['candidates']}")
    click.echo(f"Processed (fetched):  {stats['processed']}")
    click.echo(f"INCI found:           {stats['inci_found']}")
    click.echo(f"Usage found:          {stats['usage_found']}")
    click.echo(f"Description added:    {stats['desc_added']}")
    click.echo(f"Fetch failed:         {stats['fetch_failed']}")
    click.echo(f"No gain:              {stats['no_gain']}")
    click.echo(f"LLM cost:             {stats['cost']}")
    if dry_run:
        click.echo("\n(DRY RUN — nothing was saved)")
